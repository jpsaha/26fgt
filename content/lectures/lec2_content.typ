...
#exer[
  Show that the only automorphism of $QQ$ is the identity.
]
...
#exer[
  ...
  Let $K$ be an extension of $F$ of degree two.
  Prove that $K = F(alpha)$ for some $alpha in K$ with $alpha^2 in F$.
  Show that $K$ is Galois over $F$.
  What can be said about degree two extensions of a field of characteristic $2$.
]

#exer[
  Show that there exist automorphisms $sigma, tau$ of $k(x)$
  such that $sigma(x) = 1/x$ and $tau(x) = 1 - 1/x$.
  Describe the subgroup of $#math.op("Gal") (k(x) slash k)$ generated by $sigma, tau$.
]
...
#exer[
  ...
  Show that $K$ is a splitting field over $F$ for a set ${f_1, dots, f_n}$ of polynomials in 
  $F[x]$ if and only if $K$ is a splitting field over $F$ for the polynomial $f_1 dots f_n$.
]

#exer[
  Let $K$ be a splitting field of a set $S$ of polynomials over $F$.
  If $L$ is a subfield of $K$ containing $F$ for which each $f in S$ splits over $L$,
  show that $L = K$.
]

#exer[
  Provide an example of field extension $F subset.eq K subset.eq L$ such that $L slash K$ and $K slash F$ are normal, but $L slash F$ is not normal.
]

#exer[
  Let $K$ be an extension of a field $F$.
  Assume that $K$ is algebraically closed. Show that the algebraic closure of $F$
  in $K$ is an algebraic closure of $F$.
]
